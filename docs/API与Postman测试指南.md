...
| `identity_public_key` | 见当前环境（33 字节 Signal 公钥，Alice/Bob 不同） |
...
| # | 请求 | 期望 |
|---|------|------|
| 17 | PATCH /api/users/me | 200，更新用户名 |
| 18 | POST /api/users/me/avatar | 200，返回 avatar_url |

**阶段 6 Signal KDS（需 Bearer，Alice 窗口）：**

| # | 请求 | 期望 |
|---|------|------|
| 19 | PUT /api/users/me/signal-keys | 200 |
| 20 | GET /api/users/{userId}/signal-devices | 200，含 device_id |
| 21 | GET /api/users/{userId}/signal-bundle | 200，含 identity_key 与 pre_key |
...
| 23 | PATCH /api/conversations/{id} | Alice | 200，群名更新 |
| 24 | GET /api/conversations/{id}/member-directory | 任一方 | 200，成员列表含公钥 |
| 25 | POST /api/conversations/{id}/key-bundles | Alice | 201，welcome 密文 `e2ee:gw:v1:` 前缀 |
| 26 | GET /api/conversations/{id}/key-bundles | 新成员 | 200，仅含入群后 epoch 的 bundle |
| 27 | POST /api/conversations/{id}/rotate-keys | 群主 | 200，`epoch++` |

**阶段 5 个人资料（单人）：**

| # | 请求 | 期望 |
|---|------|------|
| 23 | PATCH /api/users/me | 200，用户名已更新 |

**阶段 6 Signal KDS（双账号 Alice + Bob）：**

| # | 请求 | 窗口 | 期望 |
|---|------|------|------|
| 24 | PUT /api/users/me/signal-keys | Alice | 200，`ok: true` |
| 25 | PUT /api/users/me/signal-keys | Bob | 200 |
| 26 | GET /api/users/{userId}/signal-devices | Alice 查 Bob | 200，含 `device_ids` |
| 27 | GET /api/users/{userId}/signal-bundle | Alice 查 Bob | 200，含 `identity_key`、一次性 PreKey |

环境变量 `signal_*` 已预置合法测试密钥（33/64 字节 Base64），Reset 后会恢复。

**阶段 4 扩展（群管理 API，在群已建且 `conversation_id` 有效时）：**

| # | 请求 | 期望 |
|---|------|------|
| 28 | PATCH /api/conversations/{id} | 200，群名已改 |
| 29 | GET /api/conversations/{id}/member-directory | 200，含历史成员 |
| 30 | POST /api/conversations/{id}/key-bundles | 201，上传 welcome 占位密文 |
| 31 | GET /api/conversations/{id}/key-bundles?epochs=0 | 200，Bob 可拉到 bundle |
| 32 | POST /api/conversations/{id}/rotate-keys | 200，`epoch` 递增 |
| 33 | DELETE /api/conversations/{id} | 200，群主解散（**放最后**，会清空会话） |
...
| `identity_public_key` | 注册时写入，33 字节 Signal 公钥（0x05 前缀） | 每账号独立，注册时上传 |
...
| POST | /api/users/me/avatar | Bearer JWT（multipart） |
| GET | /api/avatars/{filename} | 否 |
| PUT | /api/users/me/signal-keys | Bearer JWT |
| GET | /api/users/{userId}/signal-bundle | Bearer JWT |
| GET | /api/users/{userId}/signal-devices | Bearer JWT |
| GET | /api/users | Bearer JWT |
| GET | /api/conversations | Bearer JWT |
| POST | /api/conversations | Bearer JWT |
| PATCH | /api/conversations/{id} | Bearer JWT（群主改群名） |
| DELETE | /api/conversations/{id} | Bearer JWT（群主解散） |
| POST | /api/conversations/{id}/members | Bearer JWT（群主邀请） |
| DELETE | /api/conversations/{id}/members/{userId} | Bearer JWT（群主移除或自退） |
| POST | /api/conversations/{id}/rotate-keys | Bearer JWT（Megolm 轮换） |
| GET | /api/conversations/{id}/key-bundles | Bearer JWT |
| POST | /api/conversations/{id}/key-bundles | Bearer JWT |
| GET | /api/conversations/{id}/member-directory | Bearer JWT |

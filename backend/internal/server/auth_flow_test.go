...
func TestResetPasswordFlowIntegration(t *testing.T) {
	...
	handler := server.New(cfg, auth.NewHandler(authSvc), user.NewHandler(userRepo, cfg, nil, nil), userRepo, jwtMgr, nil, nil, nil, nil, admin.NewHandler(userRepo, nil, 30*24*time.Hour, admin.RuntimeConfigFrom(cfg)), nil, nil, nil, nil).Router()
...
func TestRefreshRejectsExpiredIdleTokenIntegration(t *testing.T) {
	...
	handler := server.New(cfg, auth.NewHandler(authSvc), user.NewHandler(userRepo, cfg, nil, nil), userRepo, jwtMgr, nil, nil, nil, nil, admin.NewHandler(userRepo, nil, cfg.RefreshTokenTTL, admin.RuntimeConfigFrom(cfg)), nil, nil, nil, nil).Router()
